...
class AnnotatorManager:
    ...
    def __init__(self, edges=None):
        ...
        # Color palette: Team 0 = red, Team 1 = blue, fallback = white
        team_palette = sv.ColorPalette.from_hex(['#FF2222', '#2266FF', '#FFFFFF'])

        # Basic annotators — players use team palette (color picked by class_id)
        self.ellipse_annotator = sv.EllipseAnnotator(color=team_palette)
        self.triangle_annotator = sv.TriangleAnnotator()
        self.label_annotator = sv.LabelAnnotator(color=team_palette, text_color=sv.Color.WHITE)
        self.box_annotator = sv.BoxAnnotator()

        # Referee uses a fixed yellow ellipse
        self.referee_ellipse_annotator = sv.EllipseAnnotator(color=sv.Color.YELLOW)
    ...
    def annotate_players(self, frame: np.ndarray, player_detections: sv.Detections,
                         distance_dict: dict = None) -> np.ndarray:
        """
        ...
        Args:
            ...
            distance_dict: Optional {tracker_id: meters_float} to show distance on label
        ...
        if distance_dict is not None:
            player_labels = [
                f'#{tid}  {distance_dict.get(int(tid), 0):.0f}m'
    ...
    def annotate_referees(self, frame: np.ndarray, referee_detections: sv.Detections) -> np.ndarray:
        ...
        if referee_detections is not None and len(referee_detections.xyxy) > 0:
            ...
            return self.referee_ellipse_annotator.annotate(frame, referee_detections)
        return frame

    def annotate_all(self, frame: np.ndarray, player_detections, ball_detections,
                     referee_detections, distance_dict: dict = None) -> np.ndarray:
        """
        ...
        Args:
            ...
            ball_detections: Ball detection results
            referee_detections: Referee detection results
            distance_dict: Optional {tracker_id: meters_float} for distance overlay
        ...
        target_frame = self.annotate_players(target_frame, player_detections, distance_dict=distance_dict)
