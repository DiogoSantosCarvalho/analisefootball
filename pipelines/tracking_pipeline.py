...
from player_clustering import ClusteringManager, ColorKMeansClusterer
...
class TrackingPipeline:
    ...
    def __init__(self, model_path, use_color_kmeans: bool = False):
        """
        Initialize the tracking pipeline with all necessary models.
        
        Args:
            model_path: Path to the YOLO detection model
            use_color_kmeans: If True, use fast color K-Means for team assignment
                              instead of SigLIP embeddings + UMAP + K-Means
        """
        ...
        self.use_color_kmeans = use_color_kmeans
    ...
    def initialize_models(self):
        ...
        if self.use_color_kmeans:
            self.color_clusterer = ColorKMeansClusterer()
            print("  -> Using fast color K-Means team assignment")
        else:
            self.clustering_manager = ClusteringManager()
            print("  -> Using SigLIP + UMAP + K-Means team assignment")
    ...
    def collect_training_crops(self, video_path):
        ...
        # Get video info to avoid requesting out-of-bound frames
        video_info = sv.VideoInfo.from_video_path(video_path)
        total_frames = video_info.total_frames
        end_frame = min(total_frames - 1, 120 * 24)
        
        # Get video frames
        frame_generator = sv.get_video_frames_generator(video_path, stride=12, end=end_frame)
    ...
    def train_team_assignment_models(self, video_path):
        ...
        if self.use_color_kmeans:
            # Train color K-Means from multiple sampled frames for robustness
            print("  Color K-Means: training on sampled frames...")
            video_info = sv.VideoInfo.from_video_path(video_path)
            total_frames = video_info.total_frames
            end_frame = min(total_frames - 1, 120 * 24)
            frame_generator = sv.get_video_frames_generator(video_path, stride=30, end=end_frame)
            
            training_data = []
            for frame in tqdm(frame_generator, desc='collecting jersey colors'):
                player_det, _, _ = self.detection_pipeline.detect_frame_objects(frame)
                if len(player_det.xyxy) > 0:
                    training_data.append((frame, player_det))
                if len(training_data) >= 20:  # 20 frames is enough for color training
                    break
            
            self.color_clusterer.train_from_frames(training_data)
    ...
    def clustering_callback(self, frame, player_detections):
        ...
        if len(player_detections.xyxy) > 0:
            if self.use_color_kmeans:
                cluster_labels = self.color_clusterer.predict(frame, player_detections)
            else:
    ...
    def convert_detection_to_tracks(self, player_detections, ball_detections, referee_detections, tracks, index):
        ...
        # Store referee tracks — use ByteTrack tracker_id if available, else sequential
        if len(referee_detections.xyxy) > 0:
            has_tracker_ids = (
                referee_detections.tracker_id is not None
                and len(referee_detections.tracker_id) == len(referee_detections.xyxy)
            )
            ids = referee_detections.tracker_id if has_tracker_ids else np.arange(len(referee_detections.xyxy))
            for tracker_id, bbox in zip(ids, referee_detections.xyxy):
    ...
    def track_realtime(self, video_path: str, display_metadata: bool = True, retrain: bool = True):
        ...
        Args:
            ...
            retrain: If False, skip training (models already trained externally)
        """
        import cv2
        
        self.initialize_models()
        
        # Train team assignment models (skip if already done externally)
        if retrain:
            print("Training team assignment models...")
            self.train_team_assignment_models(video_path)
