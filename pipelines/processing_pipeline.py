...
import pandas as pd
from utils import read_video, write_video, get_video_fps
...
class ProcessingPipeline:
    ...
    @staticmethod
    def write_video_output(frames, output_path, fps=None, input_path=None):
        """
        ...
        Args:
            ...
            fps: Frames per second for output video (if None, reads from input_path)
            input_path: Path to input video to read FPS from
        """
        if fps is None and input_path is not None:
            fps = get_video_fps(input_path)
            print(f"Using source video FPS: {fps}")
        elif fps is None:
            fps = 25
    ...
    def generate_output_path(input_path, suffix="_tracked"):
        """
        Generate output path inside the project's output/ folder.
        ...
        Returns:
            Generated output path inside output/ directory
        """
        import os
        output_dir = Path(__file__).resolve().parent.parent / "output"
        output_dir.mkdir(exist_ok=True)
        input_stem = Path(input_path).stem
        return str(output_dir / f"{input_stem}{suffix}.mp4")